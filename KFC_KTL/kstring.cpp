...
KString& KString::SetOEMToChar()
{
	if(sizeof(TCHAR) == 1)
		return OemToCharBuff(GetDataPtr(), GetDataPtr(), GetLength()), *this;

	return *this = GetOEMToChar();
}

KString& KString::SetCharToOEM()
{
	if(sizeof(TCHAR) == 1)
		return CharToOemBuff(GetDataPtr(), GetDataPtr(), GetLength()), *this;

	return *this = GetCharToOEM();
}

LPTSTR KString::GetOEMToChar(KString& RText, bool bEmptyFirst) const
{
	if(bEmptyFirst)
		RText.Empty();

	const LPTSTR pRet = RText.Extend(GetLength());

	OemToCharBuff(GetDataPtr(), pRet, GetLength());

	return pRet;
}

LPTSTR KString::GetCharToOEM(KString& RText, bool bEmptyFirst) const
{
	if(bEmptyFirst)
		RText.Empty();
	...
	CharToOemBuff(GetDataPtr(), pRet, GetLength());
...
KString& KString::TrimByZero(size_t szStart)
{
	...
	if(p = (char*)memchr(GetDataPtr() + szStart, 0, GetLength() - szStart))
		SetLeft(p - GetDataPtr());
