...
// Service manager
// ----------------
TServiceManager::TServiceManager(kflags_t flAutoOpenAccess)
{
    m_hManager = NULL;
...
void TServiceManager::Allocate(kflags_t flAccess)
{
    Release();
...
void TServiceManager::DeleteService(SC_HANDLE hService, bool bSafe)
{
    ...
    {
        INITIATE_DEFINED_CODE_FAILURE(  TEXT("Error deleting service"),
                                        GetLastError());
    }
}
